...
fig = data2_1%>% 
  plot_ly(x = ~year)%>% 
  add_trace(y = ~y, type = 'scatter', mode = 'lines', line = list(color = 'rgba(67,130,160,1)', width = 2)) %>% 
  ...
  add_trace(x = ~c(year[1], year[8]), y = ~c(y[1], y[8]), type = 'scatter', mode = 'markers', marker = list(color = 'rgba(49,130,189, 1)', size = 10))
fig%>%
...
*As of August 2017. Until that time approximately 35,500 beds have been delivered and 8,000 beds were awaiting completion.
...
data3 = data.frame(
  ...
  labels = c(
    "Public schools", "Private schools", "Elementary", "Secondary", "Combined",
    "Other","All"),
  parents = c(
    "", "",  "Public schools", "Public schools", "Public schools", "Public schools",
    "Private schools"
 ),
    v = c(98158,34576,66837,23814,6783,724,34576),
...
plot_ly(data3, ids = ~ids, labels = ~labels, parents = ~parents, type = 'sunburst')
```
